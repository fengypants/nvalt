...
/*
 * Hashing and encryption use Apple's CommonCrypto (part of libSystem), which
 * replaced the bundled Intel-only OpenSSL static libraries.
 */
...
- (NSData*)MD5Digest {
	...
	//MD5 is used here for identifiers/checksums, not for security
...
- (NSString *)encodeBase64WithNewlines:(BOOL)encodeWithNewlines {
	...
	//match OpenSSL's BIO_f_base64 output: 64-column lines, each terminated by a newline
	NSString *base64String = [self base64EncodedStringWithOptions:NSDataBase64Encoding64CharacterLineLength |
							  NSDataBase64EncodingEndLineWithLineFeed];
	return [base64String length] ? [base64String stringByAppendingString:@"\n"] : base64String;
}
...
- (BOOL)encryptAESDataWithKey:(NSData*)key iv:(NSData*)iv {
	return [self transformAESDataWithOperation:kCCEncrypt key:key iv:iv];
}

- (BOOL)decryptAESDataWithKey:(NSData*)key iv:(NSData*)iv {
	return [self transformAESDataWithOperation:kCCDecrypt key:key iv:iv];
}

//AES-256-CBC with PKCS#7 padding; byte-for-byte compatible with the OpenSSL EVP_aes_256_cbc() output of earlier versions
//this method will change the size of the data, but at large sizes that should be well within the malloc'ed block padding, anyway
- (BOOL)transformAESDataWithOperation:(CCOperation)operation key:(NSData*)key iv:(NSData*)iv {
	if ([iv length] != kCCBlockSizeAES128) {
		NSLog(@"initialization vector length was wrong size: %lu", (unsigned long)[iv length]);
		return NO;
	}
	if ([key length] != kCCKeySizeAES256) {
		NSLog(@"%@ key length was wrong size: %lu", operation == kCCEncrypt ? @"encryption" : @"decryption", (unsigned long)[key length]);
		return NO;
	}
	...
	CCCryptorStatus status = CCCrypt(operation, kCCAlgorithmAES, kCCOptionPKCS7Padding,
									 ...
									 [self mutableBytes], [self length], &movedLength);
	if (status != kCCSuccess) {
		NSLog(@"Couldn't %@ data: CommonCrypto error %d", operation == kCCEncrypt ? @"encrypt" : @"decrypt", (int)status);
		[self setLength:originalDataLength];
		return NO;
	}
	
	[self setLength:movedLength];
